...
const ApplicationReview = () => {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      ...
      const resp = await authFetch(`/api/applications?${params.toString()}`);
      if (resp.ok) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (resp.ok) {
        ...
        const appsResp = await authFetch('/api/applications');
        if (appsResp.ok) {
  ...
  const handleStatusChange = async (appId, newStatus) => {
    setSubmitting(true);
    try {
      const resp = await authFetch(`/api/applications/${appId}/status`, {
        method: 'PATCH',
  ...
  const fetchForensicData = async (passport) => {
    try {
      const [riskResp, bioResp] = await Promise.all([
        authFetch(`/api/risk-profile/${passport}`),
        authFetch(`/api/biometric/check/${passport}`)
      ]);
