...
const History = () => {
  ...
  const generateReport = useCallback(async (e = null) => {
    ...
    try {
      if (reportType === 'visa_stats') {
        ...
        const resp = await authFetch(`http://localhost:5000/api/reports/stats?${query}`);
        ...
        const [listResp, statsResp] = await Promise.all([
          authFetch(`http://localhost:5000/api/crossings?${query}`),
          authFetch(`http://localhost:5000/api/reports/stats?startDate=${filters.start}&endDate=${filters.end}&nationality=${filters.nationality}`)
        ]);
        ...
        const resp = await authFetch(`http://localhost:5000/api/reports/notifications?${query}`);
        if (resp.ok) setResults(await resp.json());
      } else if (reportType === 'biometric_audit') {
        const query = new URLSearchParams({ startDate: filters.start, endDate: filters.end, nationality: filters.nationality }).toString();
        const resp = await authFetch(`http://localhost:5000/api/reports/biometric?${query}`);
        if (resp.ok) setResults(await resp.json());
      } else {
        ...
        const resp = await authFetch(`http://localhost:5000/api/reports/applications?${query}`);
        if (resp.ok) setResults(await resp.json());
      }
    } catch (err) {
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      // Forensic Audit Log
      await authFetch('http://localhost:5000/api/audit/log', {
        method: 'POST',
