...
const Processing = () => {
  ...
  const fetchRiskLevel = async (passport) => {
    ...
    try {
      const resp = await authFetch(`/api/risk-profile/${passport}`);
      if (resp.ok) {
  ...
  React.useEffect(() => {
    const fetchHardware = async () => {
      try {
        const resp = await authFetch('/api/hardware/status');
  ...
  const handleQuickScan = async () => {
    ...
    try {
      const resp = await authFetch('/api/hardware/mrz-scan');
      if (resp.ok) {
        const data = await resp.json();
  ...
  const handleCalibrate = async () => {
    ...
    try {
      const resp = await authFetch('/api/hardware/calibrate', { method: 'POST' });
