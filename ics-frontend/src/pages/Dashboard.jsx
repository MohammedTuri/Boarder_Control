    ...
    const loadOverview = async () => {
      try {
        const [statsResp, crossingsResp, feedResp, analyticsResp] = await Promise.all([
          authFetch('/api/stats'),
          authFetch('/api/crossings'),
          authFetch('/api/mission-feed'),
          authFetch(`/api/reports/stats?startDate=${new Date(Date.now() - 86400000).toISOString().split('T')[0]}`)
        ]);
        ...
        const [integrityResp, hwResp] = await Promise.all([
          authFetch('/api/stats/integrity'),
          authFetch('/api/hardware/status')
        ]);
