...
const VisaDocument = () => {
  ...
  useEffect(() => {
    const fetchVisaData = async () => {
      try {
        ...
        const resp = await fetch(`${API_BASE_URL}/api/applications/status/${reference}`);
        if (resp.ok) {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${window.location.origin}/verify/${reference}`;
